...
pointless xdsin XDS_ASCII.HKL hklout pointless.mtz > POINTLESS.LP 2>/dev/null
rm -rf pointless.mtz
...
newcell=`awk '/Dataset ID/ {getline; getline; getline; getline; getline; printf "%3.1f %3.1f %3.1f %3.1f %3.1f %3.1f", $1, $2, $3, $4, $5, $6}' POINTLESS.LP`
...
Real space transformation:	$realspacetransform"

if [ -e symm_def ]; then
	pointless -copy xdsin XDS_ASCII.HKL hklout $xtal.indexed.mtz > POINTLESS.LP 2>/dev/null
else
	pointless xdsin XDS_ASCII.HKL hklout $xtal.indexed.mtz > POINTLESS.LP 2>/dev/null
	rm -rf XDS_ASCII.HKL
	inpfile=`wc -l XDS.INP | cut -c 1-3`
